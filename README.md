...
![Header Handler: add, overwrite, and remove request headers with shareable profiles](docs/store/screenshots/01-hero.png)
...
![Header Handler live-log side panel: a GET request to mywebsite.co.uk with chips naming each rule that matched it, such as "set test headers › +x-test-enabled" and "delete x-cache header › −x-cache"](docs/store/screenshots/03-livelog.png)
