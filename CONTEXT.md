...
- **Append** — Header operation that adds a value to a header using DNR's native comma-joined append (no custom separator); if the header is absent, behaves like Set. Once a header has an Append rule applied, DNR only permits lower-priority rules on that same header to also Append — a Set or Remove at lower priority is rejected at compile time.
